...
m1.metric("Total Records", str(len(df)))
m2.metric("Date Range", "2020 - 2026")
m3.metric("Features Used", "7")
m4.metric("Latest Close", "$" + str(round(float(df['Close'].iloc[-1]), 2)))
...
ax1.set_xticks(ax1.get_xticks()[::200])
ax1.legend()
ax1.grid(alpha=0.3)
plt.tight_layout()
st.pyplot(fig1)
...
if st.button("Predict Closing Price", use_container_width=True):
    ...
    r1.metric("Open Price", "$" + str(round(open_price, 2)))
    r2.metric("Predicted Close", "$" + str(round(prediction, 2)))
